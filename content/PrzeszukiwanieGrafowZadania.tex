...
Zakładamy, że dysponujemy globalną
tablicą booli $T$ o $|V(G)|$ elementach zainicjowaną wartością 
\textit{false}.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Procedure{RSort}{$G$: graf, $v$: wierzchołek startowy, $L$: lista}
		\While{$x \in N(v)$}
		\If {$T[x] = $ \textit{false} }
		\State $T[x] \gets$ \textit{true}
		\State TSort($G$, $x$, $L$)
...
Zauważmy, że dodanie do drzewa wierzchołka $x$, które odbywa się w linijkach..., nie może spowodować
cykli, bo w $G'$ nie istnieje łuk wchodzący do tego wierzchołka.
