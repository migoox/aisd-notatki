...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Procedure{IsPointInsidePolygon}{$X$: tablica zdarzeń, $P$: punkt}
			...
			\State $Y \gets \text{tablica odcinków przechowywana w zdarzeniu napotkania punktu $A$}$
			\State $a \gets 0$
			\State $b \gets |Y|-1$
			\State
			...
			\While{$b-a=1$}
				...
				\If{$(Y[i].\text{start}, Y[i].\text{end}, P)$ jest skrętem w lewo ($\Leftrightarrow$ $P$ jest nad $Y[i]$)}
					\State $b \gets i$
				\ElsIf{$(Y[i].\text{start}, Y[i].\text{end}, P)$ jest skrętem w prawo ($\Leftrightarrow$ $P$ jest pod $Y[i]$)}
					\State $a \gets i$
				\Else
					\If {$y_{Y[i].\text{start}} \neq y_{Y[i].\text{end}}$}
						\State \Return true \Comment{$P$ leży na boku wierzchołka}
					\EndIf
				 
					\If{$y_{Y[i].\text{start}} < y_P < y_{Y[i].\text{end}}$} 
						\State \Return true \Comment{$P$ leży na boku wierzchołka równoległym do $OY$}
					\EndIf
					
					\If{$y_{Y[i].\text{end}} < y_P$ ($\Leftrightarrow$ $P$ jest nad $Y[i]$)}
						\State $b \gets i$
					\ElsIf{$y_{Y[i].\text{start}} > y_P$ ($\Leftrightarrow$ $P$ jest pod $Y[i]$)}
						\State $a \gets i$
					\EndIf
